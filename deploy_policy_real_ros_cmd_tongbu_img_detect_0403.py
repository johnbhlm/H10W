...
from examples.H10W.utils.offset_helpers import add_left_arm_offset, subtract_left_arm_offset
...
WHITE_DESK_TORSO = 0.54
SOFA_TORSO = 00.26 - 0.02
TV_TORSO = 0.29
...
class Args:
    ...
    #pretrained_path: str = "/home/diana/intern-vla_test/starVLA/results/Checkpoints/internvla_full_data_finetune_pre18k9k"
    # pretrained_path: str = "/home/diana/intern-vla_test/starVLA/results/Checkpoints/gr00t_full_data"
    # pretrained_path: str = "/home/diana/intern-vla_test/starVLA/results/Checkpoints/gr00t_baseline_new"
    # pretrained_path: str = "/home/diana/intern-vla_test/starVLA/results/Checkpoints/gr00t_baseline_no_state_new"
    pretrained_path: str = "/home/diana/intern-vla_test/starVLA/results/Checkpoints/gr00t_data_latest_white_desk_v2_finetune"
    # pretrained_path: str = "/home/diana/intern-vla_test/starVLA/results/Checkpoints/gr00t_baseline_new_pre50k"
...
stats = load_stats("/home/diana/intern-vla_test/starVLA/results/Checkpoints/gr00t_data_latest_white_desk_v2_finetune/dataset_statistics.json")
...
def extract_explicit_hand(task_description: str):
...
def detect_object_and_suggest_hand(
    ...
    detected_hand = result.get("hand", None)
...
def preprocess_pick_instruction(
    interface,
    robot_controller,
    detector_client,
    task_description: str,
    ...
    explicit_hand = extract_explicit_hand(task_description)

    detect_ret = detect_object_and_suggest_hand(
        interface=interface,
    ...
    if not detect_ret["success"]:
        return detect_ret

    object_name = detect_ret["object_name"]
    detected_hand = detect_ret["detected_hand"]

    # 没有指定左右手的话，自动选手
    if explicit_hand is None:
        choose_ret = choose_available_hand(detected_hand)
        if not choose_ret["success"]:
            return {
                "success": False,
                "reason": choose_ret["reason"],
                "debug": detect_ret.get("detect_result", {})
            }

        final_hand = choose_ret["hand"]
        new_instruction = f"{task_description} with the {final_hand} hand"
        ...
        return {
            ...
            "detect_result": detect_ret.get("detect_result", {}),
            "reason": "no_explicit_hand_auto_assign"
        }

    # 已经指定左右手的话，保留指定的 hand，只把检测当 gate
    final_hand = explicit_hand

    return {
        "success": True,
        "hand": final_hand,
        "object_name": object_name,
        "new_instruction": task_description,   # 不改 hand
        "detect_result": detect_ret.get("detect_result", {}),
        "reason": "explicit_hand_keep_user_binding"
    }
...
def main(args: Args):
    ...
    detector_client = DetectionClient3DSync("ws://10.8.26.9:8000")
    ...
    try:
        while True:
            ...
            if "pick" in task_description.lower():
                ...
                #     save_debug=False,
                # )
                infer_ret = preprocess_pick_instruction(
                    interface=interface,
                    robot_controller=robot_controller,
                    detector_client=detector_client,
                    task_description=task_description,
            ...
            while not task_complate:
                ...
                left_arm_joints = add_left_arm_offset(left_arm_joints, interface.target_location)
                ...
                for i in range(exec_steps):
                    ...
                    left_arm = subtract_left_arm_offset(left_arm, interface.target_location)
