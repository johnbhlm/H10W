...
class M1Inference:
    def __init__(
        ...
        self.vla = InternVLA_M1.from_pretrained(policy_ckpt_path)
        if use_bf16:
    ...
    def check_task_finished(self,image: np.ndarray, instruction: str) -> bool:
        """
        使用 chat_with_M1 判断任务是否完成
        """
        ...
        reply = self.vla.chat_with_M1(
            image=img,
            text=question,
            max_new_tokens=64
        )[0].lower()

        return ("yes" in reply) or ("done" in reply) or ("finish" in reply)
