...
def execute_single_task(
    ...
    use_async_prefetch, use_rtc_fusion = get_exec_mode_flags(EXEC_MODE)
    ...
    try:
        logger.info(f"EXEC_MODE={EXEC_MODE}")
        logger.info(
            f"Runtime config: mode={EXEC_MODE}, control_dt={CONTROL_DT}, action_horizon=unknown, "
        ...
        logger.info(
            f"Runtime config: mode={EXEC_MODE}, control_dt={CONTROL_DT}, action_horizon={current_actions.shape[0]}, "
            f"async_enabled={use_async_prefetch}, rtc_enabled={use_rtc_fusion}"
        )
        ...
        while not task_complete:
            ...
            if allow_mid_chunk_switch:
                rtc_frozen = async_worker.get_recommended_frozen(CONTROL_DT, actions.shape[0], RTC_FROZEN)
                rtc_overlap = max(RTC_OVERLAP, rtc_frozen + 4)
                ...
                lead_steps = min(max(RTC_REQUEST_LEAD_STEPS, rtc_frozen + 2), max(1, exec_steps - 1))
                ...
                lead_steps = min(max(1, RTC_REQUEST_LEAD_STEPS), max(1, exec_steps - 1))
                ...
                if allow_mid_chunk_switch and async_requested and pending_request_id is not None and i >= rtc_swap_idx:
                    ...
                    if got is not None:
                        ...
                        if is_valid_chunk(next_actions):
                            current_actions = fuse_action_chunks(actions[i:], next_actions, overlap=rtc_overlap, frozen=rtc_frozen)
                            current_item_snapshot = next_chunk_item_snapshot
                            switched_mid_chunk = True
                            break

                sleep_remaining = max(0.0, CONTROL_DT - (time.time() - step_start))
            ...
            if use_async_prefetch:
                ...
                if async_requested and pending_request_id is not None:
                    ...
                    if is_valid_chunk(next_actions):
                        if use_rtc_fusion:
                            current_actions = fuse_action_chunks(actions[-1:, ...], next_actions, overlap=rtc_overlap, frozen=rtc_frozen)
                        else:
                ...
                if is_valid_chunk(next_actions):
                    if use_rtc_fusion:
                        current_actions = fuse_action_chunks(actions[-1:, ...], next_actions, overlap=RTC_OVERLAP, frozen=min(RTC_FROZEN, 1))
                    else:
                        current_actions = next_actions
                else:
                    current_actions = actions[-1:, ...]
...
def main(args: Args):
    ...
    mode_async, mode_rtc = get_exec_mode_flags(EXEC_MODE)
    logger.info(
        f"Runtime config: mode={EXEC_MODE}, control_dt={CONTROL_DT}, action_horizon=unknown, "
        f"async_enabled={mode_async}, rtc_enabled={mode_rtc}, "
        f"RTC_OVERLAP={RTC_OVERLAP}, RTC_FROZEN={RTC_FROZEN}, RTC_REQUEST_LEAD_STEPS={RTC_REQUEST_LEAD_STEPS}, "
