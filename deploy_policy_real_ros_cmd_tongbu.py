...
stats = load_stats("/home/diana/intern-vla_test/starVLA/results/Checkpoints/gr00t_full_data_pre77k5/dataset_statistics.json")
...
def model_warmup_producer(model, interface, task_description, shared):
    while not shared["stop"]:
        ...
        try:
            out = model.step(example)
            shared["first_actions"] = out["raw_actions"]
            shared["ready"] = True
            print("[INFO] Warm-up produced first actions")
            return
        except Exception:
            time.sleep(0.02)
...
def main(args: Args):
    ...
    try:
        while True:
            ...
            #     target=model_warmup_producer,
            #     args=(model, interface, task_description, shared),
            #     daemon=True,
            # )
            # warmup_thread.start()
            ...
            # ===============================
            # 进入 realtime 前，冻结 joint
            # ===============================
            # robot_status = robot_controller.get_status()
            # hold_left_joint = np.array(robot_status["leftjoint"])
            # hold_right_joint = np.array(robot_status["rightjoint"])
            ...
            while not task_complate:
                ...
                # cv2.imwrite(os.path.join(save_dir, f"{timestamp}_top.png"), img_top)
                # cv2.imwrite(os.path.join(save_dir, f"{timestamp}_left.png"), img_left)
                # cv2.imwrite(os.path.join(save_dir, f"{timestamp}_right.png"), img_right)
                ...
                both_close = (dl < 0.10 and dr < 0.10)
                ...
                # # ====== FIX 1：第一帧吃 warm-up ======
                # if shared["ready"]:
                #     actions = shared["first_actions"]
                #     shared["ready"] = False
                # else:
                #     actions = model.step(example)["raw_actions"]
